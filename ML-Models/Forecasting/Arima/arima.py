...
def process_file(filename, dataset):
    ...
    for building_name in df.columns:
        ...
        print(datetime.now(), i, '/', len(df.columns), building_name, df1.shape,"Dataset: ", dataset, flush=True)
        ...
        if (i % 1000 == 0) and (i != len(df.columns) - 1):
            res_all = []
            agg_metrics_all = []
            j += 1
        ...
        if i % 5 == 0:
            ...
            res_all_df.to_csv(f'Results/arima/forecasts/{dataset}/{save_filename}_{j}.csv', index=False)            

        

            agg_metrics_all_df = pd.concat(agg_metrics_all).round(6)            
            agg_metrics_all_df.to_csv(f'Results/arima/results/{dataset}/agg_metrics_{save_filename}_{j}.csv', index=False)            
    ...
    # return res_all_df, ts_metrics_all_df, agg_
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description = "zero-shot using arima")
    ...
    os.makedirs(f'Results/arima/forecasts/{dataset}/', exist_ok = True)
    os.makedirs(f'Results/arima/results/{dataset}/', exist_ok = True)
