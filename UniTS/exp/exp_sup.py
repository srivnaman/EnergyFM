...
def custom_print_decorator(func):
    def wrapper(*args, **kwargs):
        ...
        if 'folder' in kwargs and kwargs['folder']:
            with open(f'{kwargs["folder"]}/NEW_imput_zero.log', 'a') as log_file:
                log_file.write(text + '\n')
        if 'folder' in kwargs:
            del kwargs['folder']
        if 'file' in kwargs:
...
class Exp_All_Task(object):
    ...
    def train(self, setting):
        ...
        if self.args.pretrained_weight is not None:
            ...
            # print('loading pretrained model:', pretrain_weight_path, folder=self.path)
    ...
    def test_long_term_forecast(self, setting, test_data, test_loader, data_task_name, task_id):
        ...
        print('data_task_name: {} mse:{}, mae:{}'.format(data_task_name, mse, mae), folder=self.path)
        torch.cuda.empty_cache()
        return mse, mae
    ...
    def test_classification(self, setting, test_data, test_loader, data_task_name, task_id):
        ...
        mprecision, mrecall, mf_score, msupport = precision_recall_fscore_support(
                                trues, predictions, average='macro')
        precision, recall, f_score, msupport = precision_recall_fscore_support(
                                trues, predictions, average='binary')
        print("Accuracy : {:0.4f}, Macro Precision : {:0.4f}, Macro Recall : {:0.4f}, Macro F-score : {:0.4f}".format(
                accuracy, mprecision, mrecall, mf_score), folder=self.path)
        print("Accuracy : {:0.4f}, Precision : {:0.4f}, Recall : {:0.4f}, F-score : {:0.4f}".format(
                            accuracy, precision, recall, f_score), folder=self.path) 
        # print("Accuracy : {:0.4f}, Macro Precision : {:0.4f}, Macro Recall : {:0.4f}, Macro F-score : {:0.4f} ".format(
        #     accuracy, mprecision, mrecall, mf_score), folder=self.path)
        # print("Accuracy : {:0.4f}, Precision : {:0.4f}, Recall : {:0.4f}, F-score : {:0.4f} ".format(
        #     accuracy, precision, recall, f_score), folder=self.path)
