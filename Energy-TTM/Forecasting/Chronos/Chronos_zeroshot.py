...
def process_file(filename, dataset):
    ...
    for building_name in df.columns:
        ...
        if i % 5 == 0:
            ...
            res_all_df.to_csv(f'/home/user/energygpt/Results/chronos/forecasts/{dataset}/{save_filename}_{j}.csv', index=False)            
        

            agg_metrics_all_df = pd.concat(agg_metrics_all).round(6)            
            agg_metrics_all_df.to_csv(f'/home/user/energygpt/Results/chronos/results/{dataset}/agg_metrics_{save_filename}_{j}.csv', index=False)            
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description = "zero-shot using chronos")
    ...
    files_list = glob.glob(f'/media/user/DATA2/Dataset_V0.0/Energy-Load-Profiles/Hourly/Commercial/{dataset}/*.parquet')
    # fg = ['1', '10', '12', '18', '19', '25']

    
    os.makedirs(f'/home/user/energygpt/Results/chronos/forecasts/{dataset}/', exist_ok = True)
    os.makedirs(f'/home/user/energygpt/Results/chronos/results/{dataset}/', exist_ok = True)

    for filename in files_list:
        # if os.path.basename(filename)[:-8].split('-')[-1] in fg:
        #     print(filename, 'Done')
        #     continue
        # else:
