...
def test(args, model, criterion, device):
    ...
    result_path = "./predictions_finetuned_out_dgxh100"
    ...
    for region in os.listdir(folder_path):
        ...
        for building in os.listdir(region_path):
            ...
            if building.endswith('.csv'):
                ...
                train, temp_data = train_test_split(energy_data, test_size=0.6, shuffle=False)
                val, test_data = train_test_split(temp_data, test_size=0.8333, shuffle=False)

                dataset = TimeSeriesDataset(test_data, args["context_length"], args["prediction_length"], args["patch_stride"])
                ...
                # test loop
                for x_test, y_test in tqdm(DataLoader(dataset, batch_size=256), desc=f"Testing {building_id}", leave=False):
